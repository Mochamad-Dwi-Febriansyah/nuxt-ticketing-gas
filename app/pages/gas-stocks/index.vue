...
const { fetchGasStocks, deleteGasStock } = useAdminGasStock()
...
onMounted(async () => {
  try {
    const { data, error: fetchError } = await fetchGasStocks()
    if (fetchError) {
      error.value = fetchError.message
    } else {
      gasStocks.value = data.result // Sesuaikan response API kamu
    }
  } catch (err: any) {
...
const handleConfirmDelete = async () => {
  if (gasStockToDelete.value !== null) {
    ...
    try {
      const response = await deleteGasStock(gasStockToDelete.value)
      addNotification('success', response.data.message)
      const { data } = await fetchGasStocks()
      gasStocks.value = data.result
    } catch (err: any) {
      alert('Gagal menghapus data gas stock: ' + err.message)
    } finally {
