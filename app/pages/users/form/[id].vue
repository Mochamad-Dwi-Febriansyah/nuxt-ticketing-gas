...
const branches = ref<{ label: string, value: number }[]>([]) // Untuk opsi Select Branch
...
const schema = toTypedSchema(
  object({
    ...
    branch_id: number().nullable().default(null),
...
// ✅ Fetch Data Cabang untuk Select
const loadBranches = async () => {
  try {
    const { data } = await fetchBranches()
    branches.value = data.result.map((branch: any) => ({
      label: branch.name,
      value: branch.id
    }))
  } catch (err: any) {
...
<template>
  <section class="p-4 max-w-3xl mx-auto">
    ...
    <form @submit.prevent="onSubmit" class="space-y-4">
      ...
      <Input name="password_confirmation" label="Konfirmasi Password" type="password" :required="!isEditMode" />
      <Select
        name="role"
        label="Role"
        :options="[
